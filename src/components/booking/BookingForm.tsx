...
export function BookingForm({ businessId, businessSlug, businessName, onSubmit, isLoading = false }: BookingFormProps) {
  ...
  const handleGoogleLogin = async () => {
    try {
      const redirectUrl = `${window.location.origin}/${businessSlug}/book/callback`;
    ...
    <div className="space-y-5 animate-slide-up">
      ...
      {mode === 'phone' && (
        <>
          <button
            ...
            className="w-full py-4 px-4 bg-white border-2 border-gray-200 rounded-2xl font-medium text-gray-700 hover:bg-gray-50 hover:border-gray-300 transition-all flex items-center justify-center gap-3"
          >
      ...
      {mode === 'new_customer' && (
        <form onSubmit={handleNewCustomerSubmit}>
          ...
          <div className="space-y-4 mb-5">
            ...
            <div className="relative">
              ...
              <input
                ...
                `}
              />
              <p className="text-xs text-gray-500 mt-1 text-right">לשחזור סיסמה ותזכורות</p>
            </div>
          ...
          <button
            type="submit"
            disabled={name.trim().length < 2 || !email.includes('@') || password.length < 4 || isChecking || isLoading}
            className={`
              w-full py-4 rounded-2xl font-bold text-lg
              transition-all duration-300 btn-press
              ${name.trim().length >= 2 && email.includes('@') && password.length >= 4 && !isChecking && !isLoading
                ? 'bg-gradient-to-r from-blue-500 to-blue-600 text-white shadow-xl shadow-blue-500/30 hover:shadow-2xl hover:shadow-blue-500/40'
                : 'bg-gray-200 text-gray-400 cursor-not-allowed'
              }
            `}
